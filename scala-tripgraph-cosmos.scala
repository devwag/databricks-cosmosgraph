...
val bikeStations = sqlContext.sql("SELECT * FROM station_csv")
val tripData = sqlContext.sql("SELECT * FROM short_trip_csv")
...
val cosmosDbVertices = toCosmosDBVertices(stationGraph.vertices, "name")
...
val cosmosDbEdges = toCosmosDBEdges(stationGraph, "duration")
...
val cosmosDBConfig = Config(Map(
  "Endpoint" -> "https://<your-accnt>.documents.azure.com:443/",
